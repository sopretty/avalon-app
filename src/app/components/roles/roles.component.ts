...
export class RolesComponent implements OnInit {
  ...
  roles: {
  ...
  }[] = [
    {
      ...
      characters: [{
        ...
        name: 'merlin',
        team: 'blue',
        description: 'ROLES.merlinDescription'
      }],
    },
    {
      descriptionKey: 'ROLES.mordred',
      characters: [{
        asset: '/assets/characters/mordred.png',
        name: 'mordred',
        team: 'red',
        description: 'ROLES.mordredDescription'
      }],
    },
    {
      ...
      characters: [{
        asset: '/assets/characters/morgana.png',
        name: 'morgan',
        team: 'red',
        description: 'ROLES.morganaDescription'
      }, {
    ...
    },
    {
      ...
      characters: [{
        asset: '/assets/characters/oberon.png',
        name: 'oberon',
        team: 'red',
        description: 'ROLES.oberonDescription'
      }],
    },
  ];
  form: FormGroup;
  ...
  onCheckChange() {
    this.rolesForm.controls.forEach((control, idx) => {
      if (idx !== 0) {
        if (!this.form.valid) {
          if (!control.value) {
            control.disable();
  ...
  getCharacterTeam(index: number): { blue: number, red: number } {
    return this.roles[index].characters.reduce(
      (accRole, currRole) =>
        ({
          blue: accRole.blue + (currRole.team === 'blue' ? 1 : 0),
          red: accRole.red + (currRole.team === 'red' ? 1 : 0)
        })
      , { blue: 0, red: 0 });
  }

  addCheckRoles(): void {
    const storedRoles = JSON.parse(localStorage.getItem('roles')) || [true, false, false, false];
    this.roles.forEach((role, idx) => {
      this.rolesForm.push(
        new FormControl({ value: storedRoles[idx], disabled: idx === 0 })
      );
    });
    this.onCheckChange();
  }
  ...
  submit(): void {
    ...
    this.configService.roles = this.rolesForm.controls.reduce(
      (acc, curr, index) =>
        acc.concat((!!curr.value && !this.roles[index].disabled ? this.roles[index].characters : []))
      , []);
    localStorage.setItem('roles', JSON.stringify(this.rolesForm.controls.map(control => control.value)));
